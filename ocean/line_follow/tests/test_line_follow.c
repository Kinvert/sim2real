...
static void test_qti_normalization(void) {
    expect_near(line_follow_qti_normalize(100.0f, 100.0f, 1100.0f), 0.0f, 1e-6f,
        "calibrated white maps to zero");
    expect_near(line_follow_qti_normalize(1100.0f, 100.0f, 1100.0f), 1.0f, 1e-6f,
        "calibrated black maps to one");
    expect_near(line_follow_qti_normalize(600.0f, 100.0f, 1100.0f), 0.5f, 1e-6f,
        "calibration midpoint maps near half");
    expect_near(line_follow_qti_normalize(50.0f, 100.0f, 1100.0f), 0.0f, 1e-6f,
        "below white clamps to zero");
    expect_near(line_follow_qti_normalize(1500.0f, 100.0f, 1100.0f), 1.0f, 1e-6f,
        "above black clamps to one");
}

static void test_continuous_actions(void) {
    float actions[2] = {-2.0f, 2.0f};
    LineFollowWheelCommand cmd = line_follow_map_actions(actions, 0.5f, 0.05f, 1.0f, 1.0f);
    ...
    cmd = line_follow_map_actions(tiny, 0.5f, 0.05f, 1.0f, 1.0f);
    ...
    cmd = line_follow_map_actions(polarity, 0.35f, 0.0f, 1.0f, 1.0f);
...
static void test_track_generation(void) {
    ...
    expect_true(line_follow_track_generate_straight(&track, 1.0f, 0.006f, 1.0f),
    ...
    LineFollowNearest on_line = line_follow_track_nearest(&track, 0.0f, 0.0f);
    expect_near(on_line.distance_m, 0.0f, 1e-5f, "nearest distance is zero on centerline");
    LineFollowNearest off_line = line_follow_track_nearest(&track, 0.0f, 0.05f);
    ...
    expect_true(line_follow_track_generate(&track, &rng, LINE_FOLLOW_TRACK_RANDOM, 0.006f, 1.0f),
...
static void test_sensor_and_reward_behavior(void) {
    ...
    expect_true(line_follow_track_generate_straight(&env.track, 1.0f, env.line_width_m, env.track_bounds_m),
        "test straight track generation succeeds");
    line_follow_reset_runtime_pose(&env, 0.0f, 0.0f, 0.0f);
    ...
    expect_true(obs[1] > obs[0], "centered inner sensors see more black than outer sensors");

    line_follow_reset_runtime_pose(&env, 0.0f, 0.02f, 0.0f);
    compute_observations(&env);
    ...
    float near_reward = line_follow_compute_reward(&env, 0.02f, 0.0f, 0.0f, true, 0.1f, 0.0f);
    float far_reward = line_follow_compute_reward(&env, 0.02f, 0.04f, 0.0f, true, 0.1f, 0.0f);
